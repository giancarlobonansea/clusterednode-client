...
(function(app) {
	app.ReferenceService = (function() {
		...
		ReferenceService.prototype.iRL = function() {
			var l = {
				l0: this.RL.l0,
				l1: this.RL.l1,
				v: []
			};
			for (var i = 0; i < this.RL.v.length; i++) {
				l.v.push({
					         t: this.RL.v[i].t,
					         a: []
				         });
				for (var j = 0; j < this.RL.v[i].a.length; j++) {
					l.v[i].a.push({
						              h: this.snS.bypassSecurityTrustResourceUrl(this.RL.v[i].a[j].h),
						              d: this.RL.v[i].a[j].d
					              });
				}
			}
